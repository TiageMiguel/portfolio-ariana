    ...
    <li className={cn("group px-4", wrapperClasses, className)} {...props}>
      {isExternalLink ? (
        <Link
          ...
          hrefLang={lang}
        >
          <ItemContent title={title} description={description} badge={badge} />
        </Link>
      ) : (
        <ViewTransitionLink href={finalLink} hrefLang={lang}>
          <ItemContent title={title} description={description} badge={badge} />
        </ViewTransitionLink>
      )}
    </li>
...
function ItemContent({ title, description, badge }: ListItemProps) {
  return (
    <>
